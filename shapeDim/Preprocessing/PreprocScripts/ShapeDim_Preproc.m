...
for xx = 1:length(sub_list_big)
    ...
    %         make_list = 0;
        ...
        if make_list
            ...
            %Ask
            main = input(' which functional volumes were main task? \n please indicate as follows: [1 2 3 4] etc \n --> ');
            repeat = input(' which functional volumes were one-back/repeat task? \n please indicate as follows: [1 2 3 4] etc \n --> ');
            floc = input(' which functional volumes were face category localizer? \n please indicate as follows: [1 2 3 4] etc \n --> ');
            silh = input(' which functional volumes were silhouette localizer? \n please indicate as follows: [1 2 3 4] etc \n --> ');
            mdloc = input(' which functional volumes were multiple-demand localizer? \n please indicate as follows: [1 2 3 4] etc \n --> ');
            
            all_vols = sort([main,repeat,floc,silh,mdloc]);
            for vols = 1:length(all_vols)
                %which volume is this
                volume = sprintf('%03d',vols);
                %is this an exp of a loc volume?
                if sum(main==all_vols(vols))==1; text = [volume ' main'];
                elseif sum(repeat==all_vols(vols))==1; text = [volume ' rep'];
                elseif sum(floc==all_vols(vols))==1; text = [volume ' floc'];
                elseif sum(silh==all_vols(vols))==1; text = [volume ' silh'];
