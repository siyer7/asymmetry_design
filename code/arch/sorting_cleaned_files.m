...
clean_sig   = load(sprintf('%s_clean/sigs/%d_std%d_on_trim%s.mat', osort_dir, chanID, num_std, to_trim));

nsx2mat.data = clean_sig.data(:)';   % replace ONLY the signal
if ~exist([osort_dir '_clean/nsx2mat/'],'dir'), mkdir([osort_dir '_clean/nsx2mat/']); end
save(sprintf('%s_clean/nsx2mat/BL%d.mat', osort_dir, chanID), '-struct', 'nsx2mat', '-v7.3');
%%%%

% where your BL*.mat files live (from convertNSx_toMat)
paths = struct();
% base folder = parent of mat_files/, sort/, figs/, osort_out/
paths.basePath   = sprintf('../../results/%s/osort_mat_clean/', subj);    % must end with /
% input
