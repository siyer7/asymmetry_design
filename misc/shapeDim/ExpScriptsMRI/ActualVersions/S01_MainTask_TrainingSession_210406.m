...
% parameters for MRI task
    ...
    % total time = 13s blank start + 48*(1+1+1+3) + 8s blank end 
    %   = 309 seconds or 5:09
    %   with TR=0.8 sec this is 387 TRs.
...
%     p.imagedir=fullfile(root, sprintf('Stimuli/AmpGrid%d_full_grey/',p.ImageSet));
    ...
    if p.Training
        p.expName = sprintf('MainTaskMRI_feedback_sess%d_part%d_TRAINING',p.Session,p.Part);
    else
        p.expName = sprintf('MainTaskMRI_feedback_sess%d_part%d',p.Session,p.Part);
    end
    ...
    if p.debug  
        ...
        p.nTrials = 8;
        p.nTrials_variable = 4;
        p.nTrials_main = 4;
    elseif p.Training
        p.nTrials = 24;
        p.nTrials_variable = 8;
        p.nTrials_main = 16;
    else
        p.nTrials = 48;
    ...
    if p.debug==1
        myinds = [datasample(main_grid_inds,p.nTrials_main); var_inds];
    elseif p.Training==1
        myinds = [main_grid_inds; var_inds];
    else
