...
% change variable p.s                 catttttttttttttttttttnnerlaptop to indicate which situation.
...
try
    ...
    if p.scannerlaptop || IsWindows  
        ...
        % linux, behavior rooms - save to atttttttttttttttttttt different folder.
    ...
    dims = [1 35];
    definput = {'0','AR','10','2','1','1','0','3','9','1 '};
    ...
    p.LowContrastVersion = boolean(str2double(answer{10}));
...
%    
    ...
    % don't change this, matches t he images!
       % round(0.3*255)
    p.backColorOrig = 77;
...
%         DrawFormattedText(w, sprintf('%d',bb), p.boundTextPos(bb,1)+p.textAdjPix(1),p.boundTextPos(bb,2)+p.textAdjPix(2),p.fixColor);
    end 
...
%                 DrawFormattedText(w, sprintf('%d',bb), p.boundTextPos{oo}(bb,1)+p.textAdjPix(1),p.boundTextPos{oo}(bb,2)+p.textAdjPix(2),p.white);
            ...
            DrawFormattedText(w, showtext, 'center', p.centerPix(2)-p.feedbackTextHeightPix, p.fixColor);
            Screen('DrawDots', w, [0,0], p.fixSizePix, p.fixColor, p.centerPix, 0); 
            Screen('DrawingFinished', w);
            Screen('Flip', w);
